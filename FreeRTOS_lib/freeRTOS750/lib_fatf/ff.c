...
#else
#error Number of _VOLUMES must not be 0.
#endif
...
{
#if defined (portUSE_TIMER2) // RTC function using Timer 2
	...
	return ( (uint32_t)fatfs_time( &y2ktime ) );

#elif defined( portRTC_DEFINED ) // DS1307 I2C chip RTC
...
)
{
	...
	if (res == FR_NO_FILE) res = FR_DENIED; // patched on 15th March - elmchan web site
	return res;
}
#endif
...
#endif // define only if portSD_CARD or portEXT_RAMFS is enabled.
