...
static uint32_t reference_timestamp_sec;	// Time stamp of last NTP update in Y2K epoch
...
void	check_NTP (SOCKET s)				// Check system_time against NTP time
{
	...
	while( xTaskGetTickCount() < (xInitialTick + ( NTP_TIMEOUT / portTICK_RATE_MS )) )	// as long as time is remaining
	{
...
static void send_NTP_request (SOCKET s)
{
	...
	ip.lVal = htonl( inet_addr( (uint8_t*) NTP_SERVER_1));				// the NTP server we're using.
