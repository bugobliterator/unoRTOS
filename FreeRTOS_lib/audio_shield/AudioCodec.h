...
static inline void AudioCodec_ADC_init(void)
{
	// setup ADCs
#if ADCS == 0
...
#elif ADCS == 2
static inline void AudioCodec_ADC(uint16_t * _mod0value, uint16_t * _mod1value)
{
    if (ADCSRA & (1 << ADIF))				// check if sample ready
    {
...
static inline void AudioCodec_SPI_init(void)
{
...
static inline void AudioCodec_Timer1_init(void)
{
...
static inline void AudioCodec_data(int16_t* _lin, int16_t* _rin, int16_t _lout, int16_t _rout)
{
...
inline void AudioCodec_dsp(void); // prototype for the DSP function to be implemented.
								  // needs to at least provide left_out and right_out

// inline within Timer1 interrupt routine - time critical I/O. Keep it short and punchy!
...
#endif
	// end mark - check for end of interrupt - for debugging only - delete when not wanted
