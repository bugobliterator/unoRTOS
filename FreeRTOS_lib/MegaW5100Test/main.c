...
/* IINCHIP interface include file. */
...
uint8_t src_mac_addr[6] = 		{0x00, 0x08, 0xDC, 0x00, 0xCA, 0xFE};	// Local MAC address
...
#ifdef portW5200
	spiBegin(SDCard); // make sure the Elecrow W5200 SDCard SS line is high, to disable its MISO.
#endif
...
static void TaskDHCP(void *pvParameters) // DHCP Client
{
    ...
    while(1)
    {
		...
		if( (time(NULL) - get_NTP_reference_time() ) > NTP_REFRESH_PERIOD ) // update every 7200 seconds
		...
		vTaskDelayUntil( &xLastWakeTime, ( 50 / portTICK_RATE_MS ) );
		PORTB &= ~_BV(PORTB7);       // main (red IO_B7) LED off. EtherMega LED off
		vTaskDelayUntil( &xLastWakeTime, ( 29750 / portTICK_RATE_MS ) );
...
//	xSerialPrintf_P(PSTR(", Code page: %u\r\n"), _CODE_PAGE);
    ...
    while(1)
    {
		...
		case 't' :	/* t [<year yy> <month mm> <date dd> <day: Sun=1> <hour hh> <minute mm> <second ss>] */
			...
			time(&p1);
	    	ctime_r( (time_t *)&p1, (char *)Line );
	    	xSerialPrintf_P(PSTR("Local Time: %s\r\n"), Line);
			break;

#elif defined ( portRTC_DEFINED )
		case 't' :	/* t [<year yy> <month mm> <date dd> <day: Sun=1> <hour hh> <minute mm> <second ss>] */
		...
		case 'i' :	// XXX IP operations from here
			...
			switch (*ptr++) {
				...
				case 'i' :	/* ii - initialise for general testing - can respond to PING off this setup */
					...
					// Initialise the W5100 / W5200 & SPI bus
					IINCHIP_init();
					...
					ip_addr.lVal = htonl( inet_addr( (uint8_t*)SUBNET_IP_MASK));	// Subnet Mask saved off chip
					saveSUBR(&ip_addr);
				...
				case 't' :	/* it - Do some custom test.*/
					...
					xSerialPrintf_P(PSTR("Sn_MR = %.2x Sn_SR = %.2x Sn_CR = %.2x\r\n"), IINCHIP_read(Sn_MR(1)), IINCHIP_read(Sn_SR(1)), IINCHIP_read(Sn_CR(1)) );
					...
					vTaskDelay(  64 / portTICK_RATE_MS  );
					xSerialPrintf_P(PSTR("Sn_MR = %.2x Sn_SR = %.2x Sn_CR = %.2x\r\n"), IINCHIP_read(Sn_MR(1)), IINCHIP_read(Sn_SR(1)), IINCHIP_read(Sn_CR(1)) );
