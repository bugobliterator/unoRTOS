	...
	)
{
	...
	port = IINCHIP_read( Sn_DPORT0(s) );
	port = ((port & 0x00ff) << 8) + IINCHIP_read( Sn_DPORT1(s) );
	return port;
}
#endif
...
uint32_t GetIPAddress(void)
{
	...
	{
		ip <<= 8;
		ip += (uint8_t)IINCHIP_read(SIPR0+i);
	}
	return ip;
}
...
uint32_t GetGWAddress(void)
{
	...
	{
		ip <<= 8;
		ip += (uint8_t)IINCHIP_read(GAR0+i);
	}
	return ip;
}
...
uint32_t GetSubMask(void)
{
	...
	{
		ip <<= 8;
		ip += (uint8_t)IINCHIP_read(SUBR0+i);
	}
...
{
	for(uint8_t i=0; i<6; ++i) *mac++ = IINCHIP_read(SHAR0+i);
}

void GetDestMacAddr(SOCKET s, uint8_t* mac)
{
	for(uint8_t i=0; i<6; ++i) *mac++ = IINCHIP_read(Sn_DHAR0(s)+i);
}
