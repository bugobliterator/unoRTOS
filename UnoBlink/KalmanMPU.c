...
// from an I2C device.
//
// It uses the boolean parameter for endTransMission()
...
uint8_t i2cWrite(uint8_t registerAddress, uint8_t *data, uint8_t length, bool sendStop) {
  beginTransmission(IMUAddress);
  write(registerAddress);
  write(data, length);
  uint8_t rcode = endTransmission(sendStop); // Returns 0 on success
...
uint8_t i2cRead(uint8_t registerAddress, uint8_t *data, uint8_t nbytes) {
  ...
  beginTransmission(IMUAddress);
  write(registerAddress);
  uint8_t rcode = endTransmission(false); // Don't release the bus
  if (rcode) {
    return rcode; // See: http://arduino.cc/en/Reference/WireEndTransmission
  }
  requestFrom(IMUAddress, nbytes,(uint8_t)true); // Send a repeated start and then release the bus after reading
  for (uint8_t i = 0; i < nbytes; i++) {
    if (available())
      data[i] = read();
    else {
      timeOutTimer = micros();
      while (((micros() - timeOutTimer) < I2C_TIMEOUT) && !available());
      if (available())
        data[i] = read();
      else {
